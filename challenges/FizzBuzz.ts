...
type Fizz<A extends any[]> = A["length"] extends 0
  ? "Fizz"
  : A extends [infer _, infer _, infer _, ...infer Rest]
  ? Fizz<Rest>
  : "";
type Buzz<A extends any[]> = A["length"] extends 0
  ? "Buzz"
  : A extends [infer _, infer _, infer _, infer _, infer _, ...infer Rest]
  ? Buzz<Rest>
  : "";
type ArrayString<
  A extends any[],
  S extends string = `${Fizz<A>}${Buzz<A>}`
> = S extends "" ? `${A["length"]}` : S;
type FizzBuzz<N extends number, R extends string[] = []> = N extends R["length"]
  ? R
  : FizzBuzz<N, [...R, ArrayString<[...R, ""]>]>;
