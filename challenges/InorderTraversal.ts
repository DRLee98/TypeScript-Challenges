...
type Flatten<T> = T extends [infer F, ...infer Rest]
  ? [...(F extends any[] ? Flatten<F> : [F]), ...Flatten<Rest>]
  : [];
type InorderTraversal<T extends TreeNode | null> = T extends {
  val: infer V;
  left: infer L;
  right: infer R;
}
  ? Flatten<
      [
        L extends TreeNode ? InorderTraversal<L> : [],
        V,
        R extends TreeNode ? InorderTraversal<R> : []
      ]
    >
  : [];
